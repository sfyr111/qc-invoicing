    ...
    <el-dialog
      ...
      <el-form
        ...
        <el-form-item
          ...
          label-width="100px"
          prop="roleName">
          <el-input v-model="ruleForm.roleName" :readonly="edit"></el-input>
        </el-form-item>
        <el-form-item
          label="是否有效"
          label-width="100px">
          <el-select
            ...
            placeholder="请选择性别">
            <el-option
              v-for="isValid in isValids"
              :key="isValid.value"
              :label="isValid.text"
              :value="isValid.value"></el-option>
          </el-select>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button
          type="primary"
          @click="roleSave('ruleForm')"
          :loading="hasSubmit">保存</el-button>
      </span>
    </el-dialog>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      dialogOpen() {
      	this.$store.dispatch('clrFunList')
        this.getFunTree()
      ...
      roleEdit(row) {
        ...
        this.dialogOpen()
        this.getFunList(row.roleId)
        this.edit=true
      },
      roleSave(formName) {
      	let checked=this.$refs.tree.getCheckedNodes();
      	let checkedIds = checked.reduce(function (pre, item) {
      ...
      getFunTree() {
      	let _this = this
      	let opt = {
      		url: configUrl.getFuncTree.dataUrl,
          type: 'get',
          success: function (resp) {
        ...
        this.$store.dispatch('funTree', opt)
      },
      getFunList (roleId) {
        let _this = this
        let opt = {
        ...
        this.$store.dispatch('funList', opt)
      },
    }
  }
</script>
